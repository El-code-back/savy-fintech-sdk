...
import { Plus, X, Home, ShoppingBag, CreditCard, Shield, Gift, Car, Plane } from 'lucide-react';
...
export default function VaultsScreen() {
  const { vaults, runwayDays, addVault } = useApp();
    ...
    <div className="pb-32 pt-10 px-6">
      <header className="mb-10">
        ...
        <div className="flex justify-between items-center">
          <p className="text-[10px] font-bold tracking-widest text-[#8F8A9B] uppercase">{vaults.length} Active Goals Tracking in Real-Time</p>
          {useApp().internalDebt > 0 && (
            ...
            >
              <p className="text-[8px] font-black text-[#E84855] uppercase tracking-widest">Internal Debt: {formatKGS(useApp().internalDebt)}</p>
            </motion.div>
          )}
        </div>
      </header>
        ...
        {vaults.map((vault, idx) => {
          ...
          return (
            ...
            >
              ...
              <div className="h-1.5 w-full bg-[#302945] rounded-full mb-6 overflow-hidden">
                <motion.div
                  className={cn(
                    "h-full rounded-full",
                    isComplete ? "bg-[#2BCB8A]" : (vault.priority === 'CRITICAL' ? "bg-[#E84855]" : (vault.priority === 'HIGH' ? "bg-[#E58A32]" : "bg-[#B685FF]"))
                  )}
                  initial={{ width: 0 }}
              ...
              <ActionButton
                title={isComplete ? "RELEASE FUNDS" : "ACCELERATE GOAL"}
                variant={isComplete ? "outline" : "primary"}
                className={isComplete ? "border-[#302945] bg-[#302945]/20" : ""}
              />
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            >
              <div className="flex justify-between items-center mb-8">
                ...
                <button onClick={() => setShowAddModal(false)} className="text-[#8F8A9B]"><X size={24} /></button>
              </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {['CRITICAL', 'HIGH', 'MEDIUM', 'LOW'].map(p => (
                      <button 
                        key={p}
                        onClick={() => setNewVault({...newVault, priority: p as any})}
                        className={`p-3 rounded-xl border text-[10px] font-bold transition-all ${
                          newVault.priority === p ? 'bg-[#B685FF]/10 border-[#B685FF] text-white' : 'bg-[#110C1E] border-[#302945] text-[#8F8A9B]'
                        }`}
                      >
                        {p}
                      </button>
